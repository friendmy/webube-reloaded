...
var userRouter = _express["default"].Router(); //console.log("users");


userRouter.get("/logout", _middlewares.protectorMiddleware, _userController.logout);
userRouter.route("/edit").all(_middlewares.protectorMiddleware).get(_userController.getEdit).post(_middlewares.avatarIpload.single("avatar"), _userController.postEdit);
